...
app.get('/get-summoner', function (req, res) {
  ...
  fetch('https://na.api.pvp.net/api/lol/' + req.query.region + '/v1.4/summoner/by-name/' + name + '?api_key=' + config.RIOT_API_KEY)
...
app.get('/get-summoner-stats', function (req, res) {
  ...
  fetch('https://na.api.pvp.net/api/lol/' + req.query.region + '/v1.3/stats/by-summoner/' + req.query.summonerId + '/ranked?api_key=' + config.RIOT_API_KEY)
    .then(function(response) {
      if (response.status !== 200) {
        console.log('Looks like there was a problem. Status Code: ' + response.status)
        res.sendStatus(status)
      ...
      .then(function(champions) {
        ...
        fetch('https://na.api.pvp.net/api/lol/' + req.query.region + '/v2.2/matchlist/by-summoner/' + req.query.summonerId + '?rankedQueues=TEAM_BUILDER_DRAFT_RANKED_5x5,RANKED_SOLO_5x5,RANKED_TEAM_3x3,RANKED_TEAM_5x5&api_key=' + config.RIOT_API_KEY)
          .then(function(response) {
            if (response.status !== 200) {
              console.log('Looks like there was a problem. Status Code: ' + response.status)
              res.sendStatus(status)
...
app.get('/get-champion', function (req, res) {
  ...
  fetch('https://global.api.pvp.net/api/lol/static-data/' + req.query.region + '/v1.2/champion/' + req.query.championId + '?api_key=' + config.RIOT_API_KEY)
    .then(function(response) {
      if (response.status !== 200) {
